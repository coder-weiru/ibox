...
public class DeleteTodoHandlerTest {
    ...
    @InjectMocks
    private DeleteTodoHandler handler = new DeleteTodoHandler();

    @Mock
    private TodoDataService TodoDataServiceMock;

    public DeleteTodoHandlerTest() {
        IPlannerComponent iPlannerComponent = DaggerIPlannerComponent.builder().build();
        iPlannerComponent.inject(handler);
    }

    @Test
    public void getTodo_shouldInvokeTodoDateServiceWithTodoId() throws Exception {
        ...
        verify(TodoDataServiceMock, times(1)).deleteTodo(todoIdCaptor.capture());
    ...
    @Test
    public void getTodo_shouldReturnBadRequestMessageIfTodoIdInvalid() throws Exception {
        APIGatewayProxyRequestEvent requestEvent = new APIGatewayProxyRequestEvent();
    ...
    public void getTodo_shouldReturnInternalServerErrorMessageIfAmazonServiceExceptionIsThrown() throws Exception {
        ...
        amazonDynamoDBException.setServiceName("DeleteItem");
        amazonDynamoDBException.setRequestId("request1");
        amazonDynamoDBException.setErrorMessage("error message");

        doThrow(amazonDynamoDBException).when(TodoDataServiceMock).deleteTodo(any(String.class));
